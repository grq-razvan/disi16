 ...
 */
class SimpleRegionStochasticHillClimbingSearcher extends AbstractHillClimbingSearcher {

    //simple one = generate K initial guesses (regions), having the same distance function
    //apply HC in parallel to each
    //get result

    SimpleRegionStochasticHillClimbingSearcher(Integer maxKnapsackWeight) {
        ...
        GParsPool.withPool {
            for (int restarts = 0; restarts < runtimeParams.restarts; restarts++) {
                ...
                for (int regionIndex = 0; regionIndex < regions.size(); regionIndex++) {
                    ...
                    while (!initialKnapsack.validate() && restarts == 0) {
